...
def mge_model_counts_at_polar_points(radius_pix, angle_deg, sol):
    ...
    (as in mgefit sector photometry).
    """
...
def polar_points_to_image_xy(radius_pix, angle_deg, pa_deg, center_pix_xy):
    ...
    center_pix_xy = (xc, yc) = (x, y) = (col, row) from mgefit.
    ...
    r = np.asarray(radius_pix, float)
    th = np.deg2rad(np.asarray(angle_deg, float))
    xp = r * np.cos(th)   # along major axis
    yp = r * np.sin(th)   # along minor axis

    pa = np.deg2rad(pa_deg)

    # major-axis unit vector = (sin(pa), cos(pa))
...
def build_mge_model_image_cutout(img_shape, sol, pa_deg, center_pix_xy,
    ...
    # rotate image coordinates -> galaxy major/minor coordinates
...
def fit_mge_from_f200(
    ...
    *,
    ...
    # Store geometry consistently as (x, y)
    xc, yc = float(f.xpeak), float(f.ypeak)
    pa = float(f.pa)
    eps = float(f.eps)
    ...
    if checkplot_dir is not None:
        ...
        for a_arc in [5.0, 15.0, 30.0]:
            ...
            b_pix = a_pix * qbar
            e = Ellipse((xc, yc), width=2 * a_pix, height=2 * b_pix,
    ...
    print(f"Found galaxy center at (x, y) = ({xc:.2f}, {yc:.2f}), PA={pa:.2f} deg, eps={eps:.3f}, theta={f.theta:.2f} deg")
    ...
    if checkplot_dir is not None:
        ...
        ax.imshow(_stretch_for_display(img, goodmask=goodmask), origin="lower")
        try:
            ax.contour((~goodmask).astype(float), levels=[0.5], linewidths=0.8, alpha=0.8,
                       origin="lower", colors="orange")
        except Exception:
            pass
    ...
    if checkplot_dir is not None:
        ...
        # mge.print_contours(
        #     img, f.theta, xc, yc, m.sol, scale=pixel_scale,
...
if __name__ == "__main__":
    ...
    print("Center (x, y) [pix]:", res["center_pix"])
