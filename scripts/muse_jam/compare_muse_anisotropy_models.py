#!/usr/bin/env python3
"""
Compare the MUSE JAM free, logistic, and constant anisotropy fits.

This script:
    - loads the common MUSE kinematics and MGE model once,
    - finds the best-fit parameters for each anisotropy prescription,
      preferring a finished NPZ result and falling back to the live checkpoint,
    - computes the corresponding Vrms model and residual maps,
    - writes comparison plots for data/model/residuals, chi2, and best-fit BH mass,
    - writes a CSV summary table.

The logistic run may still be in progress. In that case the comparison uses the
current maximum-likelihood sample in the checkpoint.
"""
...
def make_model_specs() -> list[ModelSpec]:
    free_cfg = muse.Config()
    free_cfg.beta_prescription = "free"
    free_cfg.output_dir = Path(
        "/Users/mncavieres/Documents/2026-1/Sombrero_REVEAL/Data/jam_models/muse_free_beta"
    )

    logistic_cfg = muse.Config()
    logistic_cfg.beta_prescription = "logistic"
    logistic_cfg.output_dir = Path(
        "/Users/mncavieres/Documents/2026-1/Sombrero_REVEAL/Data/jam_models/muse_logistic_beta_rwalk"
    )
    logistic_cfg.sample_method = "rwalk"
    logistic_cfg.bound_method = "multi"
    logistic_cfg.walks = 32
    logistic_cfg.bootstrap = 20
    logistic_cfg.beta_min = -4.0
    logistic_cfg.beta_max = 0.99
    logistic_cfg.beta_ra_min = 0.2
    logistic_cfg.beta_ra_max = 30.0
    logistic_cfg.beta_alpha_min = 0.5
    logistic_cfg.beta_alpha_max = 5.0

    constant_cfg = muse.Config()
    constant_cfg.beta_prescription = "constant"
    constant_cfg.output_dir = Path(
        "/Users/mncavieres/Documents/2026-1/Sombrero_REVEAL/Data/jam_models/muse_constant_beta"
    )

    return [
        ModelSpec(name="free", label="Free beta", color="tab:blue", cfg=free_cfg),
        ModelSpec(name="logistic", label="Logistic beta", color="tab:orange", cfg=logistic_cfg),
        ModelSpec(name="constant", label="Constant beta", color="tab:green", cfg=constant_cfg),
    ]


def output_dir() -> Path:
    path = Path(
        "/Users/mncavieres/Documents/2026-1/Sombrero_REVEAL/Data/jam_models/muse_anisotropy_comparison"
    )
...
def save_map_comparison(kin, comparisons: list[ModelComparison], out_dir: Path) -> None:
    ...
    for col, comp in enumerate(comparisons):
        ...
        if col == 0:
            ax_data.text(0.03, 0.97, "Observed Vrms", transform=ax_data.transAxes, ha="left", va="top",
        ...
        if col == 0:
            ax_model.text(0.03, 0.97, "Best-fit model", transform=ax_model.transAxes, ha="left", va="top",
                          bbox={"boxstyle": "round", "facecolor": "white", "alpha": 0.8, "edgecolor": "none"})
        ...
        if col == 0:
            ax_resid.text(0.03, 0.97, "Residual (data - model)", transform=ax_resid.transAxes, ha="left", va="top",
                          bbox={"boxstyle": "round", "facecolor": "white", "alpha": 0.8, "edgecolor": "none"})
    ...
    fig.suptitle("MUSE JAM anisotropy comparison: data, model, and residuals", fontsize=15)
    fig.savefig(out_dir / "muse_vrms_data_model_residual_comparison.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def save_chi2_plot(comparisons: list[ModelComparison], out_dir: Path) -> None:
    ...
    fig.suptitle("MUSE JAM anisotropy model chi-square comparison", fontsize=15)
    fig.savefig(out_dir / "muse_anisotropy_chi2_comparison.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def save_bh_mass_plot(comparisons: list[ModelComparison], out_dir: Path) -> None:
    ...
    fig.savefig(out_dir / "muse_bestfit_bh_mass_comparison.png", dpi=300, bbox_inches="tight")
    plt.close(fig)


def save_summary_csv(comparisons: list[ModelComparison], out_dir: Path) -> None:
    outpath = out_dir / "muse_anisotropy_comparison_summary.csv"
...
def main() -> None:
    ...
    logging.info("Loading common MUSE kinematics and MGE inputs")
    kin = muse.load_kinematics(specs[0].cfg)
