#!/usr/bin/env python3
"""
Monitor the MUSE JAM dynesty checkpoint and regenerate live diagnostic plots.

This script is meant to run alongside nested_free_muse.py. It watches the
checkpoint file produced by the sampler, restores its current state without
resuming the run, and writes:

    - a full dynesty corner plot,
    - a dynesty traceplot,
    - a run diagnostic plot,
    - the current best-fit Vrms data/model/residual comparison,
    - a short text status summary.

By default it polls the checkpoint file repeatedly. Use --once to perform a
single restore/checkplot pass.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Restore the MUSE dynesty checkpoint and regenerate live JAM checkplots."
    )
    parser.add_argument(
        "--checkpoint",
        default=None,
...
def resolve_paths(args: argparse.Namespace, run_cfg: muse.Config | None = None):
    run_cfg = run_cfg if run_cfg is not None else muse.Config()
...
def write_status_summary(
    ...
    with outpath.open("w", encoding="utf-8") as handle:
        handle.write("MUSE JAM checkpoint status\n")
        handle.write("=========================\n\n")
...
def run_cli(run_cfg: muse.Config | None = None) -> None:
    setup_logging()
    args = build_parser().parse_args()

    run_cfg, plot_cfg, checkpoint_path = resolve_paths(args, run_cfg=run_cfg)

    logging.info("Loading static MUSE inputs")
    kin = muse.load_kinematics(run_cfg)
